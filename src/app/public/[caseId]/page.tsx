...
export default function PublicCasePage() {
  ...
  if (loading) return <div className="min-h-screen bg-floor-black flex items-center justify-center"><Loader2 className="w-6 h-6 text-signal-amber animate-spin" /></div>;
  if (!data) return <div className="min-h-screen bg-floor-black flex items-center justify-center"><p className="text-muted-steel">Case not found</p></div>;
  ...
  return (
    <div className="min-h-screen bg-floor-black">
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-text-primary">{c.title}</h1>
          <p className="text-sm text-text-secondary mt-1">{c.facility_name} · {c.shift_name}</p>
          <div className="mt-3"><PressureGauge level={PRESSURE_LABELS[c.pressure_level] ?? 'HIGH'} label="Pressure Level" /></div>
        </div>
